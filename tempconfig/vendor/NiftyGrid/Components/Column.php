...
class Column extends \Nette\Application\UI\PresenterComponent
{
	 ...
	 * @throws NiftyGrid\DuplicateEditableColumnException
	 */
	public function setDateTimeEditable()
	{
		if($this->editable){
			throw new NiftyGrid\DuplicateEditableColumnException("Column $this->name is already editable.");
		}
		$this->parent['gridForm'][$this->parent->name]['rowForm']->addText($this->name, NULL)->getControlPrototype()->addClass("grid-datetimepicker")->addClass("grid-editable");
	 ...
	 * @return Column
	 */
	public function setDateTimeFilter()
	{		
		$this->parent['gridForm'][$this->parent->name]['filter']->addText($this->name, $this->label.":")->getControlPrototype()->class("grid-datetimepicker");
		$this->filterType = FilterCondition::DATETIME;
